...
public class Mapper {
    ...
    public static void doMap(String jobName, int mapTask, String inFile, int nReduce, MapFunc mapFunc) {
        ...
        String content_str = "";
        try{
            content_str = new String(Files.readAllBytes(Paths.get(inFile)));
        }catch (IOException e){
            e.printStackTrace();
        }

        //call mapFunc
        List<KeyValue> map_list = mapFunc.map(inFile, content_str);
        List<KeyValue>[] hash_list = new List[nReduce];
        //init the list
        for(int i = 0; i < nReduce; i++)
            hash_list[i] = new ArrayList<>();

        //hash them to nReduce
        for(int i = 0; i < map_list.size(); i++){
            int index = hashCode(map_list.get(i).key) % nReduce;
            hash_list[index].add(map_list.get(i));
        }

        //write to disk
        for(int i = 0; i < nReduce; i++){
            String name = Utils.reduceName(jobName, mapTask, i);
            File tmp_file = new File(name);
            FileOutputStream outStr = null;
            try{
                outStr = new FileOutputStream(tmp_file);
                String write_cont = JSONArray.toJSONString(hash_list[i]);
                outStr.write(write_cont.getBytes());
                outStr.close();
            }
            catch (IOException e){
                e.printStackTrace();
